...
class Chart extends Component {
  ...
  activeRangeStyles = (range) =>
    this.props.stock.range.query === range ? styles.rangesActive : null;
  ...
  render() {
    ...
    const highestClosePrice = Math.max.apply(
      Math,
      _chart.map((interval) => interval.close)
    );
    const label = <VictoryLabel style={svgStyles.chartLabel} />;
    const lineSegment = (
      <LineSegment style={svgStyles.chartGrid} type={"grid"} />
    );
      ...
      <View style={{ height: 250 }}>
        <View style={styles.rangesContainer}>
          {this.state.ranges.map((range, index) => (
            <TouchableHighlight
              ...
              key={index}
              onPress={() =>
                this.props.getStockChart(quote.symbol, range.range)
              }
            ...
            >
              <Text
                style={[
                  styles.rangesLabel,
                  this.activeRangeStyles(range.range.query),
                ]}
              >
                {range.label}
              </Text>
        ...
        <View
          ...
          style={[
            styles.chart,
            {
              marginLeft: this.chartWidth(highestClosePrice, width).marginLeft,
            },
          ]}
        >
          <VictoryChart
          ...
          >
            ...
            <VictoryLine
              data={_chart}
              style={this.positiveOrNegativeOverTime(
                _chart[0].close,
                _chart[_chart.length - 1].close
              )}
              y="close"
              x="label"
            />
